...
export class MyCarousel01Service {
  private state: any;
  private slides: any;
  ...
  build(model, params) {
    ...
    return carousel.load().promise.then(
      function() {
        carousel.visible =
          carousel.visible > carousel.total ? carousel.total : carousel.visible;

        carousel.currentIndex =
          carousel.startSlide > carousel.total - 1
            ? carousel.total - 1
            : params.startSlide;
        ...
        try {
          if (carousel.visible !== 2) {
            carousel.visible =
              carousel.visible % 2 ? carousel.visible : carousel.visible - 1;
          }
        } catch (error) {
          console.log(error);
        }
  ...
  handleImageLoad(imageLocation) {
    ...
    this.percent.next(
      new Object({ percent: Math.ceil((this.loadCount / this.total) * 100) })
    );
  ...
  setSlides() {
    ...
    const rightOut = (this.leftOutSlide = this.currentIndex - num),
      leftOut = (this.rightOutSlide =
        this.total - this.currentIndex - num <= 0
          ? -(this.total - this.currentIndex - num)
  ...
  getVisibleSlidesIndex() {
    ...
    let visibleSlidesArr = Object.keys(visibleSlidesObj).map(
      k => visibleSlidesObj[k]
    );
    ...
    const indexInTab = visibleSlidesArr.findIndex(
      val => val === this.currentIndex
    );
    return visibleSlidesArr.splice(
      indexInTab - Math.floor(this.visible / 2),
      this.visible
    );
  }
