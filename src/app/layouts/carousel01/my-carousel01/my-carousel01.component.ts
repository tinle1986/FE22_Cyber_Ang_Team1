...
@Component({
  selector: "app-my-carousel01",
  templateUrl: "./my-carousel01.component.html",
  styleUrls: ["./my-carousel01.component.scss"]
})
export class MyCarousel01Component implements OnInit, OnDestroy, OnChanges {
  ...
  buildCarousel(slides, options) {
    this.$carouselService = new MyCarousel01Service(this.slides, this.options);
    this.percentSubscription = this.$carouselService
      .getPercent()
      .subscribe(sub => {
    ...
    return this.$carouselService
      ...
      .then(
        carousel => {
          ...
          jquery("div.carousel-3d-container").css({
  ...
  render(animate, speedTime) {
    ...
    const outerHeight = this.carousel3d.getOuterHeight(),
      outerWidth = this.carousel3d.getOuterWidth(),
      slideTop =
        this.carousel3d.topSpace === "auto"
          ? 0
          : outerHeight / 2 - outerHeight / 2,
      slideLeft = this.carousel3d.width / 2 - outerWidth / 2,
      speed = speedTime
        ? speedTime / 1000
        : this.carousel3d.animationSpeed / 1000;
    let zIndex = 999;
    ...
    if (this.carousel3d.total > this.carousel3d.visible) {
      const rCSS = this.setCss(
          this.carousel3d.rightSlides.length - 1,
          this.carousel3d.rightSlides.length - 1,
          true
        ),
        lCSS = this.setCss(
          this.carousel3d.leftSlides.length - 1,
          this.carousel3d.leftSlides.length - 1,
          true
        );
  ...
  setCss(i, zIndex, positive) {
    const leftRemain =
        this.carousel3d.space === "auto"
          ? (i + 1.4) * (this.carousel3d.width / 1.5)
          : (i + 1.4) * this.carousel3d.space,
      transform = positive
        ? "translateX(" +
          leftRemain +
          "px) translateZ(-" +
          (this.carousel3d.inverseScaling + (i + -0.4) * 100) +
          "px) rotateY(-" +
  ...
  goSlide(index) {
    ...
    this.carousel3d.setCurrentIndex(
      index < 0 || index > this.carousel3d.total - 1 ? 0 : index
    );
  ...
  goNext() {
    if (
      this.carousel3d.getLock() ||
      (!this.carousel3d.loop && this.carousel3d.isLastSlide())
    ) {
      return false;
    }
  ...
  goPrev() {
    if (
      this.carousel3d.getLock() ||
      (!this.carousel3d.loop && this.carousel3d.isFirstSlide())
    ) {
      return false;
    }
  ...
  goFar(index) {
    ...
    const currentIndexInVisibleSlides = visibleSlides.findIndex(
      val => val === this.carousel3d.currentIndex
    );
  ...
  getSlide(index): any {
    return index >= 0
      ? jquery(this.$slides[index])
      : jquery(this.$slides[this.carousel3d.total + index]);
  }
  ...
  private subscribe() {
    this.$timer = timer(
      this.carousel3d.autoRotationSpeed,
      this.carousel3d.autoRotationSpeed
    ).subscribe(() => {
